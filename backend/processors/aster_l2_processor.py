...
# Import the enums from the enums module
from enums import MineralIndices, GoldPathfinderIndices
...
class ASTER_L2_Processor:
    ...
    def validate_data(self, mineral: Union[MineralIndices, GoldPathfinderIndices]):
        """
    ...
    def calculate_pathfinder_index(self, pathfinder: GoldPathfinderIndices):
        ...
        try:
            ...
            if pathfinder == GoldPathfinderIndices.GOLD_ALTERATION:
                ...
                valid_mask = (band4 > 0) & (band6 > 0) & ~np.isnan(band4) & ~np.isnan(band6)
                index_map = np.zeros_like(band4)
                index_map[valid_mask] = band4[valid_mask] / band6[valid_mask]
                ...
                valid_mask = (band4 > 0) & (band8 > 0) & ~np.isnan(band4) & ~np.isnan(band8)
                index_map = np.zeros_like(band4)
                index_map[valid_mask] = band4[valid_mask] / band8[valid_mask]
                ...
                valid_mask = (band2 > 0) & (band1 > 0) & ~np.isnan(band2) & ~np.isnan(band1)
                index_map = np.zeros_like(band2)
                index_map[valid_mask] = band2[valid_mask] / band1[valid_mask]
                ...
                valid_mask = (band7 > 0) & (band8 > 0) & ~np.isnan(band7) & ~np.isnan(band8)
                index_map = np.zeros_like(band7)
                index_map[valid_mask] = band7[valid_mask] / band8[valid_mask]
                ...
                valid_mask = (band4 > 0) & (band5 > 0) & ~np.isnan(band4) & ~np.isnan(band5)
                index_map = np.zeros_like(band4)
                index_map[valid_mask] = band4[valid_mask] / band5[valid_mask]
                confidence_map = np.ones_like(index_map) * 0.8
    ...
    def calculate_mineral_index(self, mineral: Union[MineralIndices, GoldPathfinderIndices]):
        """
        ...
        """
        try:
            # Handle GoldPathfinderIndices by delegating to calculate_pathfinder_index
            if isinstance(mineral, GoldPathfinderIndices):
                return self.calculate_pathfinder_index(mineral)
    ...
    def save_mineral_map(self, mineral: Union[MineralIndices, GoldPathfinderIndices], output_dir: Path):
        """
    ...
    def resample_data(self):
        """Resample all bands to match target resolution"""
        try:
            target_shape = None
            target_band = None
